...
export default class Pokedex extends Component {
  ...
  handleSearch(input: string) {
    ...
    pokeApiService.getPokemonByName(input).then((p) => {
      if (p) {
        ...
        var newSearches = this.state.searches.slice(
          Math.max(this.state.searches.length - 2, 0)
        );
  ...
  setPokemonAndIcon(url: string) {
    ...
    pokeApiService.getPokemonById(encountered).then((p) => {
      this.setState({ pokemon: p });
        ...
        {searches.length > 0 && (
          <div className="searches">
            ...
            {searches.slice(Math.max(searches.length - 3, 0)).map((search) => (
              <ul>
                <li>
                  <a onClick={(): void => this.reDisplay(search)}>
                    {this.capitalize(search.name)}
                  </a>
                </li>
              </ul>
            ))}
